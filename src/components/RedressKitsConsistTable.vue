<template>
  <div>
    ...
    <v-text-field v-model="searchQuery" label="Search Redress Kits" prepend-inner-icon="mdi-magnify" clearable class="mb-4" @update:modelValue="filterRedressKits"></v-text-field>

    <!-- Таблица с компактной группировкой -->
    <v-data-table :group-by="groupBy" :headers="headers" :items="filteredRedressKits" item-value="item" :items-per-page="-1" height="calc(100vh - 200px)" fixed-header class="elevation-1">
      <!-- Группа redress_kit (без отдельной строки) -->
      <template v-slot:group-header="{ item, toggleGroup, isGroupOpen }">
        <tr class="group-header-row">
          <td colspan="100%">
            <div class="d-flex align-center py-2">
              <v-btn :icon="isGroupOpen(item) ? 'mdi-chevron-down' : 'mdi-chevron-right'" variant="text" @click="toggleGroup(item)"></v-btn>
